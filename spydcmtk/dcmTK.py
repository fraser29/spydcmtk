...
class DicomSeries(list):
    ...
    def splitMultiVolumeSeries(self, normalAngleTol_deg=20.0):
        ...
        # Cluster by slice normal
        self.sortByInstanceNumber()
        all_sliceNormals = []
        for k1 in range(len(self)):
            iop = self.getImageOrientationPatient_np(k1)
            sliceVec = np.cross(iop[:3], iop[3:6])
            all_sliceNormals.append(sliceVec)
        all_sliceNormals = np.array(all_sliceNormals)
        labels = dcmTools.vector_clustering(all_sliceNormals, angle_tol_deg=normalAngleTol_deg)
        out_series = []
        for iLabel in np.unique(labels):
            this_out_series = [self[i] for i in np.argwhere(labels == iLabel).flatten()]
            out_series.append(DicomSeries(this_out_series,
                                          OVERVIEW=self.OVERVIEW,
                                          HIDE_PROGRESSBAR=self.HIDE_PROGRESSBAR,
                                          FORCE_READ=self.FORCE_READ,
                                          SAFE_NAME_MODE=self.SAFE_NAME_MODE))
